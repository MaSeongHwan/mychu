...
const SignupPage = () => {
  ...
  useEffect(() => {
    const isValid =
      ...
      Object.values(formData).every((v) => (typeof v === 'boolean' ? true : v.trim() !== ''));
  ...
  return (
    ...
    >
      ...
      <div className="login-logo" onClick={() => navigate('/')}> <img src={logo} alt="WellList 홈으로" /> </div>
      <form className="signup-content" onSubmit={handleSubmit}>
        ...
        {passwordStrength.text && (
          <div className={`password-strength ${passwordStrength.level}`}>{`비밀번호 강도: ${passwordStrength.text}`}</div>
        )}
